...
DATA = ROOT / 'data'
HEADERS = {'User-Agent': 'HongshengJicang-EditorialBot/1.1'}
...
def collect_and_edit():
    ...
    for item in candidates[:100]:
        ...
        published.append({'id':'hz-'+key,'date':item.get('date') or datetime.now().strftime('%Y-%m-%d'),'title':edit_title(title),'category':cat,'keywords':kws[:10],'content':edit_body(title,text,cat,kws),'source':item.get('source','公开资料'),'source_url_internal':url,'editorial_status':'published','editorial_method':'filter-score-edit'})
        if len(published) >= 60: break
    published.sort(key=lambda x:(x['date'],x['title']), reverse=True)
    DATA.mkdir(parents=True, exist_ok=True)
    (DATA / 'editorial.json').write_text(json.dumps({'updated_at':datetime.now(timezone.utc).isoformat(timespec='seconds'),'policy':'先过滤商业广告、交易导流和第五套人民币，再抓取公开资料进行摘要、分类、关键词提取和重新编辑；网站前台不显示原文跳转链接。','published_count':len(published),'rejected_count':rejected,'items':published},ensure_ascii=False,indent=2),encoding='utf-8')
