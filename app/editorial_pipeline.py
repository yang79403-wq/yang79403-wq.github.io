...
DATA=ROOT/'data'
HEADERS={'User-Agent':'Mozilla/5.0 (compatible; HongshengJicang-EditorialBot/2.0; +public-source-editing)'}
...
def category(text,fallback):
 t=norm(text)
 rules=[('福建钱币','地域收藏'),('泉州钱币','地域收藏'),('纸币','纸币研究'),('冠号','纸币研究'),('水印','纸币研究'),('人民币','纸币研究'),('银元','银元研究'),('袁大头','银元研究'),('龙洋','银元研究'),('机制币','机制币研究'),('铜元','机制币研究'),('纪念币','纪念币资讯'),('金银币','金银币资讯'),('金币','金银币资讯'),('银币','金银币资讯'),('版别','版别研究'),('鉴赏','鉴赏研究'),('古钱币','钱币知识'),('古钱','钱币知识'),('秦半两','钱币知识'),('五铢','钱币知识'),('开元通宝','钱币知识')]
 for k,c in rules:
  if k in t:return c
 return fallback
...
def fetch_article(url):
 try:
  ...
  return norm(' '.join(parts[:24]))[:7500]
 except Exception:return ''

def edit_title(title):
 title=repair(title);title=re.sub(r'[!！?？]+','',norm(title));title=re.sub(r'^(震撼|重磅|速看|注意)[:： ]*','',title)
 return title[:58].rstrip('，。,:：')

def edit_body(title,raw,cat,kws):
 sentences=re.split(r'(?<=[。！？])\s*',repair(raw));useful=[norm(s) for s in sentences if len(norm(s))>=25 and not blocked(s)][:5]
 focus='、'.join(kws[:6]) or '钱币收藏、历史资料、鉴赏研究'
 overview=' '.join(useful)[:720] if useful else f'本条资料围绕“{title}”展开，重点涉及{focus}。洪盛集藏按公开资料重新整理，便于收藏者从历史背景、钱币特征与研究线索等角度阅读。'
 if cat in ('银元研究','机制币研究'): observe='重点观察钱文与图案布局、压力细节、边齿、铸造或打制痕迹、磨损和包浆，并与可靠图录或同版实物进行对照。'
 elif cat in ('纸币研究',): observe='重点核对版式、冠号、水印、印刷工艺、纸张状态和保存痕迹，避免把单一特征当作版别结论。'
 elif cat in ('钱币知识','版别研究'): observe='从时代、钱文、形制、版式、铸造工艺和历史背景建立交叉验证框架。'
 elif cat=='地域收藏': observe='结合福建及泉州等地域历史、流通背景、地方铸币与收藏文化进行交叉研究。'
 else: observe='重点核对发行背景、规格材质、图案设计、历史资料和公开信息来源，区分事实与市场观点。'
 return {'智能导读':f'洪盛集藏智能编辑：{overview}','核心分析':f'本条资料的研究关键词为“{focus}”。先确认对象、时代和品类，再把可观察特征与可靠资料逐项对应，避免只凭单一特征下结论。','鉴赏观察':observe,'收藏提示':'公开资料存在来源差异，市场说法、图片状态和个体差异均可能影响判断。本页用于学习、研究与收藏交流参考，不替代专业鉴定、评级或交易结论。','编辑方式':'自动采集 → 正文抽取 → 广告与第五套人民币过滤 → 品类分类 → 智能摘要与研究提示。'}
...
def collect_and_edit():
 ...
 for item in candidates[:120]:
  ...
  published.append({'id':'hz-'+key,'date':item.get('date') or datetime.now().strftime('%Y-%m-%d'),'title':edit_title(title),'category':cat,'keywords':kws[:10],'content':edit_body(title,text,cat,kws),'source':repair(item.get('source','公开资料')),'source_url_internal':url,'editorial_status':'published','editorial_method':'auto-collect-filter-edit'})
  if len(published)>=60:break
 published.sort(key=lambda x:(x['date'],x['title']),reverse=True)
 output={'updated_at':datetime.now(timezone.utc).isoformat(timespec='seconds'),'policy':'自动采集、正文抽取、广告过滤、第五套人民币过滤、品类分类、智能摘要与研究提示；前台不显示外部原文链接。','published_count':len(published),'rejected_count':rejected,'items':published}
