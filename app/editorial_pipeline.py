...
DATA = ROOT / 'data'
HEADERS = {'User-Agent': 'HongshengJicang-EditorialBot/1.3'}
...
def repair_mojibake(s):
    ...
    try:
        ...
        return fixed if sum(fixed.count(m) for m in markers) < sum(s.count(m) for m in markers) else s
...
def category(text, fallback):
    t=norm(text)
    rules=[('福建钱币','地域收藏'),('泉州钱币','地域收藏'),('金银币','金银币资讯'),('金币','金银币资讯'),('银币','金银币资讯'),('纪念币','纪念币资讯'),('银元','银元研究'),('袁大头','银元研究'),('龙洋','银元研究'),('机制币','机制币研究'),('铜元','机制币研究'),('古钱币','钱币知识'),('古钱','钱币知识'),('秦半两','钱币知识'),('五铢','钱币知识'),('开元通宝','钱币知识'),('纸币','纸币研究'),('冠号','纸币研究'),('水印','纸币研究'),('人民币','纸币研究'),('版别','版别研究'),('鉴赏','鉴赏研究')]
    for k,c in rules:
        if k in t: return c
    return fallback
...
def edit_title(title):
    title=repair_mojibake(title); title=re.sub(r'[!！?？]+','',norm(title)); title=re.sub(r'^(震撼|重磅|速看|注意)[:： ]*','',title)
    return title[:54].rstrip('，。,:：')+'｜收藏资料整理' if len(title)>54 else title
...
def collect_and_edit():
    try: raw=repair_tree(json.loads((DATA/'news.json').read_text(encoding='utf-8')))
