...
const SYSTEM = `你是“洪盛集藏 AI 收藏智能体”。你负责收藏学习、钱币研究、数字档案和福建地域收藏知识。回答时优先依据洪盛集藏检索上下文；如果没有资料，明确说明需要进一步检索，不要编造。涉及真伪、评级、成交价格时必须说明不确定性，不替代正式鉴定或交易报价。`;
...
export async function onRequest(context) {
  ...
  if (!env.DASHSCOPE_API_KEY) return json({ ok:false, error:'AI服务尚未配置，请在 Cloudflare Secret 中设置 DASHSCOPE_API_KEY。', code:'MODEL_NOT_CONFIGURED' },503,headers);
  ...
  const messages = [{role:'system',content:SYSTEM + contextText}, ...history, {role:'user',content:message}];
  ...
  try {
    response = await fetch('https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions', {
      ...
      body:JSON.stringify({model:env.DASHSCOPE_MODEL || MODEL,messages,temperature:0.25,max_tokens:1200}),
      signal:ctrl.signal
    });
  } catch (e) {
    clearTimeout(timer);
    return json({ok:false,error:e?.name==='AbortError'?'AI请求超时，请稍后重试。':'AI服务暂时不可用。',code:'UPSTREAM_ERROR'},502,headers);
  }
  clearTimeout(timer);
